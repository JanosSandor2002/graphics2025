...
{
    SDL_Event event;
    int x, y;  // Deklaráljuk a 'x' és 'y' változókat
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_KEYDOWN:
            ...
            case SDL_SCANCODE_S:
                set_left_pad_speed(&(game->pong), +100);
                break;
            case SDL_SCANCODE_EQUALS:  // Ha az = billentyűt (és +) nyomjuk
                // Növeld a labda sebességét
                game->pong.ball.speed_x *= 1.1;  // Labda sebességének növelése
                game->pong.ball.speed_y *= 1.1;
                break;
            case SDL_SCANCODE_KP_PLUS:  // Ha a numerikus billentyűzeten lévő + billentyűt nyomjuk
                // Növeld a labda sebességét
                game->pong.ball.speed_x *= 1.1;  // Labda sebességének növelése
                game->pong.ball.speed_y *= 1.1;
                break;
        ...
        case SDL_MOUSEMOTION:
            // Itt deklaráljuk az 'x' és 'y' változókat, hogy helyesen működjön
            SDL_GetMouseState(&x, &y);  // 'x' és 'y' értékei beállítódnak
            set_right_pad_position(&(game->pong), y);  // A 'y' érték használata
            break;
        case SDL_QUIT:
            game->is_running = false;
            break;
